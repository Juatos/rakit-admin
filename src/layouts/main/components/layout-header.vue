  ...
  <n-layout-header bordered class="h-50px">
    <n-flex justify="space-between" align="center" class="h-full px-4">
      <n-flex align="center" :size="6">
        ...
        >
          <ra-icon :name="menus.isCollapsed ? 'ra:menu-right' : 'ra:menu-left'" size="22" />
        </div>
          ...
          >
            ...
            >
              <n-flex align="center" :size="4">
                <ra-icon v-if="item.icon" :name="item.icon" size="16" />
        ...
        <template v-else>
          <n-tooltip placement="bottom" :delay="200">
            <template #trigger>
              <div
                ...
                @click="toggleTheme()"
              >
                <ra-icon :name="isDark ? 'ra-ani:moon' : 'ra-ani:sun'" size="20" />
              </div>
            </template>
          ...
          <n-tooltip placement="bottom" :delay="200">
            <template #trigger>
              ...
              >
                <ra-icon :name="isFullscreen ? 'ra:screen-exit' : 'ra:screen-full'" size="20" />
              </div>
            </template>
          ...
          <!--                  <ra-icon name="ra-ani:bell" size="20" /> -->
          <!--                </n-badge> -->
          <!--              </div> -->
          <!--            </template> -->
          <!--            <span>查看消息</span> -->
          ...
          <!--          <div class="flex-center h-32px w-32px cursor-pointer hover:bg-[var(&#45;&#45;hover-color)] rounded-full"> -->
          <!--            <ra-icon name="ra:setting" size="20" /> -->
          <!--          </div> -->
          <!--        </template> -->
...
/* 面包屑图标动画 */
:deep(.n-breadcrumb-item .ra-icon) {
  transition: transform 0.15s ease;
}

:deep(.n-breadcrumb-item:hover .ra-icon) {
